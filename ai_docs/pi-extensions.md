> Source: [Pi v0.85.1 / packages/coding-agent/docs/extensions.md](https://github.com/earendil-works/pi/blob/v0.85.1/packages/coding-agent/docs/extensions.md). Copied from the installed package.
...
See [examples/extensions/](pi-extension-examples-index.md) for working implementations.
...
To share extensions via npm or git as pi packages, see [packages.md](pi-packages.md).
...
See [Session Format](pi-session-format.md) for session storage internals and the SessionManager API.
...
Fired on compaction. See [compaction.md](pi-compaction.md) for details.
...
Fired on `/tree` navigation. See [Sessions](pi-sessions.md) for tree navigation concepts.
...
Fired before each LLM call. Modify messages non-destructively. See [Session Format](pi-session-format.md) for message types.
...
Transforms chain across handlers. See [input-transform.ts](https://github.com/earendil-works/pi/blob/v0.85.1/packages/coding-agent/examples/extensions/input-transform.ts) and [input-transform-streaming.ts](https://github.com/earendil-works/pi/blob/v0.85.1/packages/coding-agent/examples/extensions/input-transform-streaming.ts) for `streamingBehavior`-aware routing.
...
`true` in TUI and RPC modes. `false` in print mode (`-p`) and JSON mode. Use this to guard dialog methods (`select`, `confirm`, `input`, `editor`) and fire-and-forget methods (`notify`, `setStatus`, `setWidget`, `setTitle`, `setEditorText`) that work in both TUI and RPC modes. In RPC mode, some TUI-specific methods are no-ops or return defaults (see [rpc.md](pi-rpc.md#extension-ui-protocol)).
...
Read-only access to session state. See [Session Format](pi-session-format.md) for the full SessionManager API and entry types.
...
See [dynamic-tools.ts](example-extension-dynamic-tools.md) for a full example.
...
See [send-user-message.ts](example-extension-send-user-message.md) for a complete example.
...
Register a keyboard shortcut. See [keybindings.md](pi-keybindings.md) for the shortcut format and built-in keybindings.
...
Set the model for the current session. The change is recorded in session history and restored when that session is resumed, but it does not change the configured `defaultProvider` or `defaultModel` used by new sessions. Returns `false` if authentication is not configured for the model's provider. See [models.md](pi-models.md) for configuring custom models.
...
See [custom-provider.md](pi-custom-provider.md) for advanced topics: custom streaming APIs, OAuth details, model definition reference.
...
**Early termination:** Return `terminate: true` from `execute()` to hint that the automatic follow-up LLM call should be skipped after the current tool batch. This only takes effect when every finalized tool result in that batch is terminating. See [examples/extensions/structured-output.ts](example-extension-structured-output.md) for a minimal example where the agent ends on a final structured-output tool call.
...
See [examples/extensions/tool-override.ts](example-extension-tool-override.md) for a complete example that overrides `read` with logging and access control.
...
See [Shell tool session environment](pi-environment-variables.md#shell-tool-session-environment) for variable semantics. See [examples/extensions/ssh.ts](https://github.com/earendil-works/pi/blob/v0.85.1/packages/coding-agent/examples/extensions/ssh.ts) for a complete SSH example with `--ssh` flag.
...
See [examples/extensions/truncated-tool.ts](https://github.com/earendil-works/pi/blob/v0.85.1/packages/coding-agent/examples/extensions/truncated-tool.ts) for a complete example wrapping `rg` (ripgrep) with proper truncation.
...
Tools can provide `renderCall` and `renderResult` for custom TUI display. See [tui.md](pi-tui.md) for the full component API and [tool-execution.ts](https://github.com/earendil-works/pi-mono/blob/main/packages/coding-agent/src/modes/interactive/components/tool-execution.ts) for how tool rows are composed.
...
For the exhaustive list of keybinding ids and defaults, see [keybindings.md](pi-keybindings.md). `keybindings.json` uses those same namespaced ids.
...
**For custom components, see [tui.md](pi-tui.md)** which has copy-paste patterns for:
...
See [examples/extensions/timed-confirm.ts](https://github.com/earendil-works/pi/blob/v0.85.1/packages/coding-agent/examples/extensions/timed-confirm.ts) for complete examples.
...
See [tui.md](pi-tui.md) for the full component API.
...
See [tui.md](pi-tui.md) for the full `OverlayOptions` and `OverlayHandle` API and [overlay-qa-tests.ts](https://github.com/earendil-works/pi/blob/v0.85.1/packages/coding-agent/examples/extensions/overlay-qa-tests.ts) for examples.
...
See [tui.md](pi-tui.md) Pattern 7 for a complete example with mode indicator.
...
All render functions receive a `theme` object. See [themes.md](pi-themes.md) for creating custom themes and the full color palette.
...
| RPC (`--mode rpc`) | `"rpc"` | `true` | Dialogs and notifications via JSON protocol; `custom()` returns `undefined`. See [rpc.md](pi-rpc.md) |
...
All examples in [examples/extensions/](pi-extension-examples-index.md).
