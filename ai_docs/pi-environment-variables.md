> Source: [Pi v0.85.1 / packages/coding-agent/docs/environment-variables.md](https://github.com/earendil-works/pi/blob/v0.85.1/packages/coding-agent/docs/environment-variables.md). Copied from the installed package.
...
Provider API-key variables are documented separately in [Providers](pi-providers.md#environment-variables-or-auth-file).
...
| `PI_HARDWARE_CURSOR` | Set to `1` to show the hardware cursor; see [Terminal setup](pi-terminal-setup.md) |
...
Provider credentials such as `ANTHROPIC_API_KEY`, `OPENAI_API_KEY`, and cloud-provider configuration are listed in [Providers](pi-providers.md#environment-variables-or-auth-file).

`PI_SERVER_DIR` and `PI_SERVER_ID` apply only to the source-only [experimental remote harness](pi-development.md#experimental-remote-harness), not distributed builds.
